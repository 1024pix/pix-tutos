...
async function main() {
  ...
  const raTutosURL = `https://tutos-pr${prNumber}.review.pix.fr`;
  const prGithubURL = `https://github.com/1024pix/pix-tutos/pull/${prNumber}`;

  const scalingoCommentRegex = new RegExp(raTutosURL, 'i');
...
async function getBranchNameFromPRNumber(prNumber) {
  const { data } = await axios(
    `https://api.github.com/repos/1024pix/pix-tutos/pulls/${prNumber}`
  );
