...
RA_TUTOS_URL="https://tutos-pr$PR_NUMBER.review.pix.fr"

MESSAGE_PREFIX="I'm deploying this PR to these urls:"

existing_comments=$(curl -Ssf -u $GITHUB_USER:$GITHUB_USER_TOKEN \
	"https://api.github.com/repos/1024pix/pix-tutos/issues/${PR_NUMBER}/comments")
...
if [[ $existing_comments == *"${MESSAGE_PREFIX}"* ]]; then
	...
	curl -Ssf -u $GITHUB_USER:$GITHUB_USER_TOKEN \
		-X POST "https://api.github.com/repos/1024pix/pix-tutos/issues/${PR_NUMBER}/comments" \
    --data "{\"body\":\"$MESSAGE_PREFIX\n\n- Pix Tutos : $RA_TUTOS_URL\n\n Please check it out!\"}"
fi
